...
import com.incquerylabs.magicdraw.plugin.metamodelreport.dtos.ProfileCoverageDTO;
...
public class ProfileCoverageAction extends CoverageReportAction {
	...
	public Result actionPerformed() {
		...
		Collection<Property> propertysOfStereotyps = getPropertiesOfStereotypes(stereotypes);
...
//					}
	...
	private Collection<Property> getPropertiesOfStereotypes(Collection<Stereotype> stereotypes) {
		return stereotypes.stream().flatMap(str -> str.getOwnedAttribute().stream())
				.filter(prop -> prop.getVisibility() == VisibilityKindEnum.PUBLIC && !prop.getName().startsWith("base_")).collect(Collectors.toList());
	}
	...
	public void calculatePropertyCoverage(Map<Property, Collection<Element>> propertyCoverageMap, ProfileCoverageDTO profileCoverageBean) {
		...
		for (Entry<Property, Collection<Element>> properties : propertyCoverageMap.entrySet()) {
			Property prop = properties.getKey();
			String propName = prop.getQualifiedName();
